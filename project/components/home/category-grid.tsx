          ...
          {categories.map((category) => {
            ...
            return (
              <Link
                key={category.name}
                href={category.href}
                className="group rounded-lg border p-3 sm:p-4 hover:bg-muted/50 transition-colors"
              >
                <div className="flex flex-col items-center gap-1.5 sm:gap-2 text-center">
                  <Icon className={`h-5 w-5 sm:h-6 sm:w-6 ${category.color}`} />
                  <span className="text-xs sm:text-sm font-medium group-hover:text-primary">
                    {category.name}
                  </span>
                </div>
              </Link>
